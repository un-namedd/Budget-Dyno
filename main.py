...
    """
    Remove a role from a user in the current guild.

    This command can only be run by users with administrator permissions.
    """
    guild: discord.Guild = interaction.guild

    if not interaction.user.permissions_in(guild).administrator:
        await interaction.response.send_message("You do not have the necessary permissions to run this command.", ephemeral=True)
        return

    if role in user.roles:
        await user.remove_roles(role, reason="Role removed via slash command")
        await interaction.response.send_message(f"Successfully removed role {role.name} from {user.name} in {guild.name}", ephemeral=True)
    else:
        await interaction.response.send_message(f"{user.name} does not have the role {role.name} in {guild.name}", ephemeral=True)
...
    """
    Add a role to a user in the current guild.
...
        await user.add_roles(role, reason="Role added via slash command")
        await interaction.response.send_message(f"Successfully added role {role.name} to {user.name} in {guild.name}", ephemeral=True)
    else:
        await interaction.response.send_message(f"{user.name} already has the role {role.name} in {guild.name}", ephemeral=True)
